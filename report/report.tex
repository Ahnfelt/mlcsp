...
sequence, all processes running concurrently must finish before the next process
can take any steps.
...
separated by commas. It runs the processes concurrently and returns when they have all 
...
are then run concurrently, and there is no way to get a handle to a running process
through our API.
...
$(unit \to \alpha) \times (unit \to \beta)$, runs the two processes concurrently,
...
need to add {\it parallel} and hereby achiving a hierarchical model, processes
with subprocesses.
...
Both C++CSP and JCSP support output guards and refer to \emph{PRI ALT} in their
\emph{priSelect} definition. We choose not to offer \emph{priSelect} since it's
semantics are unclear.

% har ikke kigget på de to næste afsnit

We also don't implement {\it FAIR SELECT} because it's not really implemented in
Occam and both the versions from JCSP and C++CSP2 differs from each other but
both agree on that over time as our implementation, will ensure, statistically,
that all the process will be called the same amount of time and no process can
...
to recieve a chunck of data, if it block the I/O until it has recieved the data
or if another process that is also waiting for a chunck.

We discovered that Ocaml support non-blocking I/O so it will be no problem have
...
code file after a second of waiting (\emph{Thread.sleep}). As we can see in the test
...
\newpage
\section{Appendix: Why systhreads? (Xavier Leroy)}
