...
standard OCaml library. However, that library is based on \cite{concurrentml}
and uses a model with first class composable events. It does not provide any
...
To avoid that kind of undefined behavior we do not provide handles to 
...
side effects are desirable (if ever). Our parallel construct looks like this:
...
Even though the construct is called \emph{parallel}, there is no guarantee that
they actually run in parallel on the hardware. In particular, our implementation
does not support this. However, it does guarantee that the processes will be run
concurrently.
...
it may get a handle to via OCaml's standard scoping rules or any other way a
...
synchronization beyond the scope of this library. There seems to be no obvious
...
possible to handle exceptions from each thread separately. However, this fine
...
side of the question mark introduces a binding. It is also not desirable to
redefine
...
extensible inheritance hierarchy for channels, we choose functions for the task.
...
separated by $|$. The semantics is that once one or more of the
...
indefinitely, and thus be starved.
...
If multiple processes repeatedly wait to act on the same channel, and one
process is chosen every time,
the other processes will be starved. We solve this by serving processes waiting
on any given channel
on a first come, first served basis.

Note that this is also a valid behavior of the arbitrary choice, which we don't
provide separately. Providing this as the arbitrary choice alternation is
...
repeatedly, but it becomes less and less likely that it will be starved in this
sense as the number of calls to select grows. Polling in a loop like this may
...
ready. The obvious
way to define a select with no guards at all is therefore to block indefinitely,
causing a deadlock.
...
is necessary to propagate the poison to the other processes in the network. As
...
In many cases this is the desired behavior. JCSP and C++CSP2 do not provide it,
...
functions can add permissions. See section \ref{implementation} for the complete
set.
...
A \emph{one-to-one} channel is a channel that only allows one process to read
from it
and one process 
...
write again before the channel was ready to accommodate the starving process.
The starving process
would always be put behind the eager process with a stack, but with a queue, the starving process
will always advance in the queue every time a message is
transmitted, guaranteeing that it will eventually get its turn.
...
\bibitem[Concurrent ML]{concurrentml}
:\\
