...
? Compare more with Occam. (context???)\\
...
Otherwise it calls \emph{attempt} on each of them. The \emph{attempt} function
may succeed, carrying out a transmission and returning $Some\ v$ where v is the
transmitted value, or it may fail, returning $None$. If any of them succeeds,
the corresponding guarded process is executed and the select returns the result
of the guard. For readers, the guarded process is passed the read value.

If none of the attempts succeed, it \emph{subscribes} each guarded process,
adding the appropriate queue on the associated channel. Then it enters a loop
where it waits on the process' condition variable each iteration. When it is
woken up, it checks if it has been involved in a transmission, and if so
returns the resulting value. 

We use a lock that is global to the library, in order to protect the channel and
processes from concurrent mutation. This lock is only taken when first trying to
read from a channel, or right after a process has been woken up. We claim that
this won't lead to performance degradation, since OCaml's threads only provides
concurrency though time-sharing \cite{ocaml-threads}.
...
\missing{
- priSelect is a part of the impl. of select.\\
}
