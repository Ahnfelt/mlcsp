...
The goals to this project were:
\begin{itemize}
 \item Desing and implement an API for a CSP-based concurrency library in a
   functional language, in this case OCaml.
 \item Be able to evaluate the API, by implementing an application on top of it.
   And compare the library with other existing CSP libraries, with emphasis on
   design, composability, extendability and usability.
 \item Deliver a short, at most one page, but complete documentation to the API.
\end{itemize}

We are satisfied with the result of the design and afterwards implementation. 
Even though at the very beginning discovered that we could not achieve true
paralelism, we still believe that our library is the one that is closest to
the theoretical aspects of CSP. If we look on how Brian Vinter, professor at
Copenhagen University, teaches CSP\cite{vintercsp} through practical simple
networks:
\begin{verbatim}
PairsInt (in, out) = Delta2Int (in, a, c) || 
                     ...
                     PlusInt (b, c, out) 
\end{verbatim}
versus how we implement the network:
\begin{verbatim}
let pairsInt i o () =
...
and definition of the channel variables. Besides this, we offer a library that
is typed. We can check at compiler time if the network actually can run, this
is an advantage over other libraries, such as PyCSP and JCSP, this last one
chooses to send \emph{objects} over channels to afterwards \emph{cast} them,
which gives no sense at all if you have a typed language.

%TODO: evaluation, talk a bit of proxy

%TODO: our documentation is so nice in comparsion to JCSP :)

%TODO: already library to conccurency in event? why not wrapper on it ... cos
%we build on csp and they on concurrent ml, already in intro :/

%TODO: idea on how to achieve true paralelism through sockets, but future ...
